...
const DepartmentModal = ({ dept, onClose, hideComradeLabel }: DepartmentModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-[2147483647] bg-black/45 backdrop-blur-md flex items-center justify-center p-0 animate-in fade-in duration-300 font-beausans">
                ...
                <div className="w-full">
                  {(dept.details.use2x2Layout || dept.details.useCollageStaggered4) && dept.details.mainItems ? (
                    <div className="flex flex-col gap-4 w-full px-4">
                      {dept.details.mainItems.map((item, i) => (
                        <div key={i} className="relative aspect-[4/3] rounded-2xl overflow-hidden border border-white/10 shadow-lg">
                          <Image quality={100} src={encodeURI(item.img)} fill sizes="100vw" className="object-cover" alt="" />
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="flex flex-col gap-8">
                      {/* 1. Show Leader if exists */}
                      {dept.details.leader && (
                        <div className="flex flex-col items-center">
                          <div className="relative w-full aspect-[3/4] rounded-3xl overflow-hidden border border-white/10">
                            <Image quality={100} src={encodeURI(dept.details.leader.img)} fill sizes="100vw" className="object-cover" style={{ objectPosition: dept.details.leader.objectPosition || 'center' }} alt="leader" />
                          </div>
                          {!hideComradeLabel && (
                      ...
                      {/* 2. Show Deputies BEFORE group ONLY IF there is NO leader */}
                      {!dept.details.leader && dept.details.deputies && dept.details.deputies.map((deputy, i) => (
                        <div key={`top-${i}`} className="flex flex-col items-center">
                          <div className="relative w-full aspect-[3/4] rounded-3xl overflow-hidden border border-white/10">
                            <Image quality={100} src={encodeURI(deputy.img)} fill sizes="100vw" className="object-cover" style={{ objectPosition: deputy.objectPosition || 'center' }} alt="deputy" />
                          </div>
                          {!hideComradeLabel && (
                            <div className="mt-4 text-center">
                      ...
                      {dept.details.group && (
                        <div className="flex flex-col items-center">
                          <div className="relative w-full aspect-[16/10] rounded-3xl overflow-hidden border border-white/20">
                            <Image quality={100} src={encodeURI(dept.details.group.img)} fill sizes="100vw" className="object-cover" alt="group" />
                          </div>
                          <div className="mt-4 text-center">
                            <h3 className="text-white font-bold text-lg whitespace-pre-line">{dept.details.group.name}</h3>
                          </div>
                        </div>
                      )}

                      {/* 4. Show Deputies AFTER group ONLY IF there IS a leader */}
                      {dept.details.leader && dept.details.deputies && dept.details.deputies.map((deputy, i) => (
                    ...
                    {page.items.map((item, iIdx) => (
                      <div key={iIdx} className="flex flex-col items-center">
                        <div className="relative w-full aspect-[3/4] rounded-2xl overflow-hidden border border-white/10 shadow-lg">
                          <Image quality={100} src={encodeURI(item.img)} fill sizes="100vw" className="object-cover" style={{ objectPosition: item.objectPosition || 'center' }} alt="" />
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-8 h-[min(620px,68vh)] max-w-[1000px] mx-auto w-full">
                      {dept.details.mainItems.slice(0, 4).map((item, i) => (
                        <div key={i} className="relative rounded-[2.5rem] overflow-hidden group border-2 border-white/10 shadow-2xl">
                          <Image priority quality={100} src={encodeURI(item.img)} fill sizes="500px" className="object-cover transition-transform duration-1000 group-hover:scale-110" style={{ objectPosition: item.objectPosition || 'center', transform: item.scale ? `scale(${item.scale})` : 'none' }} alt="" />
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="w-full space-y-2">
                      ...
                      <div className="flex gap-2 h-[min(300px,32vh)]">
                        <div className="relative flex-[1] rounded-[2.5rem] overflow-hidden group border-2 border-white/10 shadow-2xl">
                          <Image priority quality={100} src={encodeURI(dept.details.mainItems[3].img)} fill sizes="500px" className="object-cover transition-transform duration-1000 group-hover:scale-105" style={{ objectPosition: dept.details.mainItems[3].objectPosition || 'center', transform: dept.details.mainItems[3].scale ? `scale(${dept.details.mainItems[3].scale})` : 'none' }} alt="" />
                        </div>
                        <div className="relative flex-[1.4] rounded-[2.5rem] overflow-hidden group border-2 border-white/10 shadow-2xl">
                          <Image priority quality={100} src={encodeURI(dept.details.mainItems[4].img)} fill sizes="700px" className="object-cover transition-transform duration-1000 group-hover:scale-105" style={{ objectPosition: dept.details.mainItems[4].objectPosition || 'center', transform: dept.details.mainItems[4].scale ? `scale(${dept.details.mainItems[4].scale})` : 'none' }} alt="" />
                        </div>
                        <div className="relative flex-[1.4] rounded-[2.5rem] overflow-hidden group border-2 border-white/10 shadow-2xl">
                          <Image priority quality={100} src={encodeURI(dept.details.mainItems[5].img)} fill sizes="700px" className="object-cover transition-transform duration-1000 group-hover:scale-105" style={{ objectPosition: dept.details.mainItems[5].objectPosition || 'center', transform: dept.details.mainItems[5].scale ? `scale(${dept.details.mainItems[5].scale})` : 'none' }} alt="" />
                        </div>
                      </div>
                    </div>
                  ) : (
                      ...
                      {dept.details.leader && (
                        <div className="w-[300px] xl:w-[350px] shrink-0 flex flex-col justify-between group">
                          <div className="relative w-full h-[min(450px,50vh)] xl:h-[min(550px,60vh)] rounded-[2.5rem] overflow-hidden border-2 border-white/10 group-hover:border-[#EE0033]/50 transition-all duration-500 shadow-2xl">
                            <Image quality={100} src={encodeURI(dept.details.leader.img)} fill sizes="350px" className="object-cover" style={{ objectPosition: dept.details.leader.objectPosition || 'center' }} alt="leader" />
                          </div>
                          {!hideComradeLabel && (
                      ...
                      {dept.details.group && (
                        <div className={`flex flex-col items-center group ${(!dept.details.leader && (!dept.details.deputies || dept.details.deputies.length === 0)) ? 'w-full max-w-[1200px]' : 'flex-1 min-w-[300px]'}`}>
                          <div className={`relative w-full rounded-[3rem] overflow-hidden border-4 border-white/20 group-hover:border-white/40 transition-all duration-500 shadow-2xl ${(!dept.details.leader && (!dept.details.deputies || dept.details.deputies.length === 0)) ? 'h-[min(650px,75vh)]' : 'h-[min(450px,50vh)] xl:h-[min(550px,60vh)]'}`}>
                            <Image priority quality={100} src={encodeURI(dept.details.group.img)} fill sizes="1200px" className={dept.details.group.objectFit === 'contain' ? 'object-contain' : 'object-cover'} style={{ objectPosition: dept.details.group.objectPosition || 'center' }} alt="group" />
                          </div>
                          <div className="mt-6 text-center">
                            <h3 className="text-white font-bold text-xl tracking-wide whitespace-pre-line">{dept.details.group.name}</h3>
                          </div>
                        </div>
                      )}
                            ...
                            return (
                              <div key={i} className="flex flex-col items-center group relative">
                                ...
                                <div className={`relative w-full ${photoHeight} rounded-[2.5rem] overflow-hidden border-2 border-white/10 group-hover:border-[#EE0033]/50 transition-all duration-500 shadow-xl`}>
                                  <Image quality={100} src={encodeURI(deputy.img)} fill sizes="350px" className="object-cover" style={{ objectPosition: deputy.objectPosition || 'center' }} alt="deputy" />
                                </div>
                                {!showNameAbove && !hideComradeLabel && (
                                  <div className="mt-4 text-center">
                                    <h4 className="text-white font-bold text-xl tracking-wide">Đồng chí <br /> {deputy.name.replace(/Đồng chí |ĐỒNG CHÍ /ig, '')}</h4>
                      ...
                      <div className="w-full flex flex-col items-center">
                        ...
                        {page && page.layout === 'collage-staggered-4' ? (
                          <div className="flex gap-4 w-full h-[min(620px,68vh)] max-w-[1300px]">
                            ...
                            <div className="relative flex-[1.8] rounded-[3rem] overflow-hidden border-2 border-white/10 shadow-2xl group">
                              <Image quality={100} src={encodeURI(page.items[0].img)} fill sizes="800px" className="object-cover transition-transform duration-1000 group-hover:scale-105" style={{ objectPosition: page.items[0].objectPosition || 'center', transform: `scale(${page.items[0].scale || 1}) scaleX(${page.items[0].scaleX || 1})` }} alt="" />
                            </div>
                            ...
                            <div className="flex-[1.2] flex flex-col gap-4">
                              <div className="flex-[0.4] flex gap-4">
                                <div className="relative flex-[0.8] rounded-[2.5rem] overflow-hidden border-2 border-white/10 shadow-xl group">
                                  <Image quality={100} src={encodeURI(page.items[1].img)} fill sizes="400px" className="object-cover transition-transform duration-1000 group-hover:scale-105" style={{ objectPosition: page.items[1].objectPosition || 'center', transform: page.items[1].scale ? `scale(${page.items[1].scale})` : 'none' }} alt="" />
                                </div>
                                <div className="relative flex-[1.2] rounded-[2.5rem] overflow-hidden border-2 border-white/10 shadow-xl group">
                                  <Image quality={100} src={encodeURI(page.items[2].img)} fill sizes="600px" className="object-cover transition-transform duration-1000 group-hover:scale-105" style={{ objectPosition: page.items[2].objectPosition || 'center', transform: page.items[2].scale ? `scale(${page.items[2].scale})` : 'none' }} alt="" />
                                </div>
                              </div>
                              <div className="relative flex-[0.6] rounded-[3rem] overflow-hidden border-2 border-white/10 shadow-xl group">
                                <Image quality={100} src={encodeURI(page.items[3].img)} fill sizes="800px" className="object-cover transition-transform duration-1000 group-hover:scale-105" style={{ objectPosition: page.items[3].objectPosition || 'center', transform: page.items[3].scale ? `scale(${page.items[3].scale})` : 'none' }} alt="" />
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="flex gap-8 w-full h-[min(620px,68vh)] max-w-[1200px]">
                            ...
                            <div className="relative flex-[1.4] rounded-[3rem] overflow-hidden border-2 border-white/10 shadow-2xl group">
                              <Image quality={100} src={encodeURI(page.items[0].img)} fill sizes="800px" className="object-cover transition-transform duration-1000 group-hover:scale-105" style={{ transform: page.items[0].scale ? `scale(${page.items[0].scale})` : 'none' }} alt="" />
                            </div>
                            ...
                            <div className="flex-[0.6] flex flex-col gap-8">
                              <div className="relative flex-1 rounded-[2.5rem] overflow-hidden border-2 border-white/10 shadow-xl group">
                                <Image quality={100} src={encodeURI(page.items[1].img)} fill sizes="400px" className="object-cover transition-transform duration-1000 group-hover:scale-105" style={{ transform: page.items[1].scale ? `scale(${page.items[1].scale})` : 'none' }} alt="" />
                              </div>
                              <div className="relative flex-1 rounded-[2.5rem] overflow-hidden border-2 border-white/10 shadow-xl group">
                                <Image quality={100} src={encodeURI(page.items[2].img)} fill sizes="400px" className="object-cover transition-transform duration-1000 group-hover:scale-105" style={{ transform: page.items[2].scale ? `scale(${page.items[2].scale})` : 'none' }} alt="" />
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-8 h-[min(620px,68vh)] max-w-[1000px] mx-auto w-full">
                            {page.items.slice(0, 4).map((item, i) => (
                              <div key={i} className="relative rounded-[2.5rem] overflow-hidden group border-2 border-white/10 shadow-2xl">
                                <Image quality={100} src={encodeURI(item.img)} fill sizes="500px" className="object-cover transition-transform duration-1000 group-hover:scale-110" style={{ objectPosition: item.objectPosition || 'center', transform: item.scale ? `scale(${item.scale})` : 'none' }} alt="" />
                              </div>
                            ))}
                          </div>
                        ) : page ? (
                            ...
                            {page.items.map((item, i) => (
                              <div key={i} className="flex flex-col items-center group">
                                <div className="relative w-full aspect-[3/4] rounded-[2rem] overflow-hidden border-2 border-white/10 group-hover:border-[#EE0033]/50 transition-all duration-500">
                                  <Image quality={100} src={encodeURI(item.img)} fill sizes="400px" className="object-cover" style={{ objectPosition: item.objectPosition || 'center' }} alt="" />
                                </div>
                                {!hideComradeLabel && item.name && (
                                  <div className="mt-6 text-center">
